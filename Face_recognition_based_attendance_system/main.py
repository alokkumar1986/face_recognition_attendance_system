...
conn = sqlite3.connect("../myapp/db.sqlite3") 
# cursor = conn.cursor()
# cursor.execute("SELECT name FROM members_members WHERE id='1';")
...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        with open('StudentDetails/StudentDetails.csv', 'a+') as csvFile:
            ...
            writer.writerow(row)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO students (serialno, id, name) VALUES (?, ?, ?)", 
               (serial, Id, name))            
            cursor.execute("INSERT INTO members_members (id, name) VALUES (?, ?)", 
               (Id, name))
            conn.commit()
            conn.close()
        csvFile.close()
...
def TrackImages():
    ...
    if exists:
        ...
        with open("Attendance/Attendance_" + date + ".csv", 'a+') as csvFile1:
            ...
            cursor = conn.cursor()
            cursor.execute("INSERT INTO members_attendance (serialno,  name, adate, atime) VALUES (?, ?, ?, ?)", 
               (str(ID), bb, str(date), str(timeStamp)))
            conn.commit()
            conn.close()
        csvFile1.close()
    else:
        ...
        with open("Attendance/Attendance_" + date + ".csv", 'a+') as csvFile1:
            ...
            writer.writerow(attendance)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO attendance (serialno, id, name, adate, atime) VALUES (?, ?, ?, ?, ?)", 
               (str(ID), str(ID), bb, str(date), str(timeStamp)))
            conn.commit()
            conn.close()
        csvFile1.close()
...
frame1 = tk.Frame(window, bg="#ffa500")
frame1.place(relx=0.11, rely=0.17, relwidth=0.40, relheight=0.80)

frame2 = tk.Frame(window, bg="#ffa500")
